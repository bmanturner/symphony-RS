...
pub struct QaRunReport {
    ...
    pub cancellations: Vec<QaCancellationObserved>,
}
...
/// check (SPEC v2 §4.5 / Phase 11.5) aborted a QA dispatch.
///
/// Pairs the request's work-item id and tracker-facing identifier with
/// the [`CancelRequest`] observed in the shared [`CancellationQueue`].
/// QA observes the run keyspace first and falls back to the
/// `work_item_id` keyspace so a parent cancel that has not yet been
/// fanned out by the propagator still short-circuits a fresh dispatch.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct QaCancellationObserved {
    /// Durable work item id the parked request carries.
    pub work_item_id: WorkItemId,
    /// Tracker-facing identifier (e.g. `PROJ-42`).
    pub identifier: String,
...
pub struct ScopeContendedObservation {
    ...
    /// `Identifier(identifier)` otherwise. QA dispatches today always
    /// reserve a run row, but the field stays optional so the runner
    /// does not silently break if a future caller emits a run-less
    /// request.
    pub run_id: Option<RunRef>,
...
pub struct QaDispatchRunner {
    ...
    /// against `(run_id, work_item_id)` after scope acquisition and
    /// before lease acquisition; an observed cancel aborts the dispatch,
...
impl QaDispatchRunner {
    ...
    /// `work_item_id`. On a [`CancellationDecision::Abort`] outcome the
    /// runner drops the just-acquired scope permits and the local
    /// capacity permit, calls `sink.mark_cancelled` when a `run_id` is
        ...
        for req in work {
            ...
            if let Some(wiring) = self.cancellation.as_ref() {
                let observed: Option<CancelRequest> = {
                    ...
                    from_run.or_else(|| q.pending_for_work_item(req.work_item_id).cloned())
                };
                if let Some(cancel_req) = observed {
                        ...
                        && let Err(err) = wiring.sink.mark_cancelled(run_id, &cancel_req)
                    {
                        warn!(
                            ...
                            run_id = %run_id,
                            error = %err,
                            "qa runner: cancellation status sink failed; relying on operator re-issue",
                        );
                    }
                    if let Some(run_id) = req.run_id {
                    ...
                    debug!(
                        ...
                        "qa runner: pre-lease cancel observed; aborted dispatch",
                    );
                    report.cancelled_before_lease += 1;
                    report.cancellations.push(QaCancellationObserved {
...
mod tests {
    ...
    fn req_full_with_run(id: i64, identifier: &str, role: &str, run_id: i64) -> QaDispatchRequest {
        QaDispatchRequest {
            work_item_id: WorkItemId::new(id),
            identifier: identifier.into(),
            title: format!("title {identifier}"),
    ...
    /// `run_id` (QA always reserves a run row in production wiring).
    #[tokio::test]
    async fn qa_broadcaster_dedups_repeated_contention_to_one_event_per_episode() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        dispatcher.enable_gate();
        ...
        queue.enqueue(req_full_with_run(1, "PROJ-1", "qa", 11));
        queue.enqueue(req_full_with_run(2, "PROJ-2", "qa", 22));
        ...
        match &persisted[0] {
            OrchestratorEvent::ScopeCapReached {
                ...
                assert_eq!(*run_id, Some(22), "QA dispatches carry run_id");
                assert!(
                    identifier.is_none(),
    ...
    /// receives.
    #[tokio::test]
    async fn qa_broadcaster_emits_on_bus_in_lockstep_with_persistence() {
    ...
    #[tokio::test]
    async fn qa_idle_pass_terminates_episode_and_re_contention_re_emits() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        ...
        queue.enqueue(req_full_with_run(3, "PROJ-3", "qa", 33));
        let _ = runner.run_pending(CancellationToken::new()).await;
    ...
    #[tokio::test]
    async fn cancellation_observed_for_run_aborts_qa_dispatch_and_drains_run_entry() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        ...
        assert_eq!(report.cancellations[0].identifier, "PROJ-1");
    ...
    #[tokio::test]
    async fn cancellation_observed_via_work_item_keyspace_aborts_qa_but_leaves_entry() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        let cancel_queue = Arc::new(Mutex::new(CancellationQueue::new()));
        let sink = Arc::new(RecordingCancellationSink::new());
        ...
        queue.enqueue(req_with_run(1, "PROJ-1", 99));
        ...
        // Work-item entry preserved for the propagator.
        let q = cancel_queue.lock().await;
        assert!(q.pending_for_work_item(WorkItemId::new(1)).is_some());
        assert!(q.pending_for_run(RunRef::new(99)).is_none());
    }

    #[tokio::test]
    async fn cancellation_without_run_reservation_skips_sink_and_run_drain_qa() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        let cancel_queue = Arc::new(Mutex::new(CancellationQueue::new()));
        let sink = Arc::new(RecordingCancellationSink::new());
        ...
        // No run_id — sink not called, work-item entry preserved.
        queue.enqueue(req(1, "PROJ-1"));
    ...
    #[tokio::test]
    async fn cancellation_skipped_for_qa_when_queue_has_no_match() {
        let queue = Arc::new(QaDispatchQueue::new());
        let dispatcher = Arc::new(RecordingDispatcher::new(QaDispatchReason::Passed));
        let cancel_queue = Arc::new(Mutex::new(CancellationQueue::new()));
        let sink = Arc::new(RecordingCancellationSink::new());
