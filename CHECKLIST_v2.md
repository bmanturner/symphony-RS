...
- [x] Add global/role/agent/repository concurrency limits. (Decomposed below.)
  ...
  - [x] Wire `ConcurrencyGate` into `FollowupApprovalRunner`, `BudgetPauseRunner`, and `RecoveryRunner` under the same contract — or, per runner, document in code comments and an ADR in `ARCHITECTURE_v2.md` why a given runner is exempt (e.g. recovery dispatches are themselves runs and re-enter the gate via the dispatched runner). Tests cover the chosen behavior for each runner. (Decomposed below.)
...
- [x] Add retry policy with max retries, budget awareness, durable budget pauses, and `BudgetExceeded` events. (Decomposed below.)
...
- [x] Add a typed `RunStatus` enum in `symphony-core` with variants `Queued`, `Running`, `Completed`, `Failed`, `Cancelled`, `CancelRequested`, `as_str()` round-trip, `is_terminal()`, and a `valid_transition(from, to)` table covering legal cooperative-cancel transitions. Landed as `crates/symphony-core/src/run_status.rs`.
- [x] Add a DB CHECK constraint on `runs.status` and a typed `update_run_status_typed` API that flows the untyped wrapper through the typed gate, with `StateError::InvalidRunTransition`. [1820 tests]
...
- [ ] Wire cancellation observation into all dispatch runners (specialist, integration, QA, follow-up approval, budget pause): observe pending cancel before lease acquisition and between agent steps; on observe, release lease + scope permits + workspace claim and mark the run `Cancelled` via the typed status API.
  ...
  - [x] Wire the specialist runner to call `observe_for_run_or_parent` before lease acquisition and between agent steps; on `Abort`, release scope permits + lease guard, mark the run `Cancelled` via the typed status gate, and `drain_for_run` only after release. Pre-lease observation landed: extended `SpecialistDispatchRequest` with `parent_work_item_id`, added a kernel-side `RunCancellationStatusSink` trait + `RunCancellationStatusError`, plumbed `SpecialistRunner::with_cancellation` (queue + sink) and a new `run_pending` step between scope acquisition and lease acquisition. On observed `Abort` the runner drops scope permits + capacity permit, calls the sink to flow the run through the typed status gate, drains only the run-keyed entry (parent entry preserved for the propagator), and surfaces `cancelled_before_lease` + a `CancellationObserved` per request on `SpecialistRunReport`. Tests cover: run-keyed cancel aborts and drains run entry; parent-keyspace cancel aborts but leaves work-item entry pending; missing run reservation skips sink + run drain; subjectless request bypasses observation entirely; sink failure still drains queue; freed scope permit lets a subsequent capped-role dispatch proceed in the same pass. Between-agent-step checks remain a follow-up because they require Dispatcher-level cooperation.
  - [x] Wire the integration runner to the same primitive (parent context = the integration's parent work-item id). Pre-lease observation landed: extended `IntegrationDispatchRunner` with `with_cancellation(queue, sink)`, added `IntegrationCancellationObserved` and `IntegrationRunReport.{cancelled_before_lease, cancellations}`, and inserted the run-or-parent observation step between scope and lease acquisition. On `Abort` the runner drops scope + capacity permits, calls the typed-status sink (only when the request reserved a `run_id`), drains the run-keyed queue entry (parent-keyed entries preserved for the propagator), and surfaces the observation on the report. Reuses `RunCancellationStatusSink` from `specialist_runner` so the composition root wires a single sink across runners. Tests cover: run-keyed cancel aborts and drains; parent-keyed cancel via `parent_id` aborts but leaves work-item entry pending; missing run reservation skips sink + run drain; unrelated cancel proceeds normally; sink failure still drains queue and records the observation. Between-agent-step checks remain a follow-up (Dispatcher-level cooperation). [1852 tests]
...
- [ ] Add the specific `OrchestratorEvent` variants required to make Phase 13's deterministic scenarios pass. Drive the variant set from test consumers, not from an enumeration of categories.
